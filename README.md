...
# AI Stack-chan Ex (フォーク版)

> **Note**: このリポジトリは [ronron-gh/AI_StackChan_Ex](https://github.com/ronron-gh/AI_StackChan_Ex) からフォークして作成されました。  
> ronron-ghさんの素晴らしい機能拡張に心から感謝いたします。

## このフォークで追加した機能

### 🎨 画像説明機能の実装
GPT-4 Visionを活用した画像説明機能を追加しました。
- **お絵かき機能**: Webブラウザ上のキャンバスで描画した絵をAIが説明
  - 自動画像リサイズ・JPEG圧縮により大きな画像にも対応
  - カスタム質問の入力に対応
- **画像アップロード**: 既存の画像ファイルをアップロードしてAIに説明してもらえる
- **会話継続モード**: 画像に関する追加質問が可能（音声入力対応）

### ⚡ メモリ最適化とUX改善
- **ストリーミングBase64エンコード**: 3KBチャンク処理により、ピークメモリ使用量を大幅削減（4.67MB → 1MB未満）
- **5MB画像対応**: 従来の2MB制限から5MBまでの大きな画像に対応
- **待機音声フィードバック**: 
  - 画像アップロード中（1-3秒）
  - Base64エンコード中（3-10秒）
  - LLM推論中（5-30秒）
  - 全処理段階で待機音声を再生し、ユーザーに進行状況を伝達
- **STT即座フィードバック**: 音声認識開始時に即座に視覚・音声フィードバック

### 🛠️ 技術詳細
- 待機音声タスクを`Robot.cpp`に統合し、重複実装を削除
- WebAPIの画像ハンドリングを改善（自動リサイズ、圧縮フォールバック）
- アイドルフレーズのキャッシュシステム（SDカードに保存）
...
---
**Table of Contents**
- [このフォークで追加した機能](#このフォークで追加した機能)
- [オリジナル版について](#オリジナル版について)
- [開発環境](#開発環境)
- [基本的な利用方法](#基本的な利用方法)
...
## その他の機能
### 画像説明機能
Webブラウザからお絵かきしたり画像をアップロードすると、GPT-4 Visionが画像を説明してくれます。
- **お絵かき機能**: ブラウザ上のキャンバスで描画し、AIに説明してもらえます
  - 自動画像リサイズ・圧縮により大きな画像にも対応
  - カスタム質問にも対応
- **画像アップロード**: 既存の画像ファイルをアップロードして説明してもらえます
- **メモリ最適化**: ストリーミングBase64エンコードにより、5MBまでの大きな画像に対応
- **待機音声フィードバック**: 画像処理中（アップロード・エンコード・AI推論）に待機音声を再生し、ユーザーに進行状況を伝えます

使い方:
1. アプリ切り替えで「画像説明モード」を選択
2. ブラウザで `http://<IPアドレス>/image_upload_page` にアクセス
3. お絵かきするか画像をアップロード
...
| 2 | 画像説明 | Webブラウザからお絵かきや画像アップロードでAIに説明してもらえます。<br>ボタンA：最後の画像を再処理<br>ボタンB：音声で追加質問<br>ボタンC：履歴クリア | 画面タッチでも音声入力可能 |
| 3 | ポモドーロタイマ | 25分のアラームと5分のアラームを交互に繰り返すアプリです。<br>ボタンA：スタート/ストップ<br>ボタンC：無音モード解除/設定 | 初期状態は無音モードです。|
| 4 | デジタルフォトフレーム | SDカードのフォルダ "/app/AiStackChanEx/photo" に保存したJPEGファイルをLCDに表示します。<br>ボタンA：次の写真を表示 <br>ボタンC：スライドショー開始 | ・SDカードに保存するJPEGファイルはサイズを320x240にしておく必要があります。<br>・開発中、SDカードがマウント不可になり、再フォーマットしないと復旧しない事象が発生していました。改善したつもりですが、念のためSDカードのデータはバックアップをお願いします。|
| 5 | ステータスモニタ | 各種システム情報を表示します。| |
